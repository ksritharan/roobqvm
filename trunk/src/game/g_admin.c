...
g_admin_cmd_t g_admin_cmds[ ] = 
  {
	...
	{"listplayersranks", G_admin_listplayersranks, "LISTPLAYERSRANKS",
      "display the ranks of connected players",
      ""
    },

	{"listranks", G_admin_listranks, "LISTRANKS",
      "display the list of possible ranks",
      ""
    },
...
//TheRoob's Addition
	{"ranktest", G_admin_ranktest, "RANKTEST",
	 "Displays your rank(s).",
	 ""
	},
	...
	{"rw", G_admin_rw, "RW",
       "Sets gamemode into rantwrestle or normal mode",
	   "[^3mapname^7] (^51|0^7)"
    },
...
static void admin_default_levels( void )
{
  ...
  Q_strncpyz( g_admin_levels[ 0 ]->flags, 
    "LISTPLAYERS ADMINTEST HELP SPECME TIME REGISTER", 
    sizeof( l->flags ) );

  Q_strncpyz( g_admin_levels[ 1 ]->name, "^5Server Regular",
    sizeof( l->name ) );
  Q_strncpyz( g_admin_levels[ 1 ]->flags, 
    "LISTPLAYERS ADMINTEST HELP SPECME TIME RANKTEST", 
    sizeof( l->flags ) );
  ...
  Q_strncpyz( g_admin_levels[ 3 ]->name, "^2Member",
    sizeof( l->name ) );
  Q_strncpyz( g_admin_levels[ 3 ]->flags, 
    "LISTPLAYERS ADMINTEST HELP SPECME TIME PUTTEAM SPEC999 KICK MUTE WARN "
    "DENYBUILD ADMINCHAT SEESFULLLISTPLAYERS REGISTER RANKTEST",
    sizeof( l->flags ) );

  Q_strncpyz( g_admin_levels[ 4 ]->name, "^3Trusted Member",
    sizeof( l->name ) );
  Q_strncpyz( g_admin_levels[ 4 ]->flags, 
    "LISTPLAYERS ADMINTEST HELP SPECME TIME PUTTEAM SPEC999 KICK MUTE SHOWBANS "
	"RANKTEST BAN NAMELOG WARN DENYBUILD REGISTER SETLEVEL ADMINCHAT "
	"SEESFULLLISTPLAYERS",
    sizeof( l->flags ) );

  Q_strncpyz( g_admin_levels[ 998 ]->name, "^1Angel",
    sizeof( l->name ) );
  Q_strncpyz( g_admin_levels[ 998 ]->flags, 
    "ALLFLAGS -INCOGNITO -DBUILDER -BANIMMUNITY -RW -GRAVITY -KNOCKBACK",
    sizeof( l->flags ) );

    Q_strncpyz( g_admin_levels[ 999 ]->name, "^1G^5o^6d",
    sizeof( l->name ) );
  Q_strncpyz( g_admin_levels[ 999 ]->flags, 
    "ALLFLAGS -DBUILDER",
    sizeof( l->flags ) );
}
...
qboolean G_admin_ranktest( gentity_t *ent, int skiparg )
{
  int i = 0;
  qboolean found = qfalse;
  int count = 0;
  ...
  if( !ent )
  {
    ADMP( "^3!ranktest: ^7you are on the console.\n" );
    return qtrue;
  }
  for( i = 0; i < MAX_ADMIN_ADMINS && g_admin_admins[ i ]; i++ )
  {
  ...
  if( found )
  {
	...
	{
		strcat(ranks, "^7an ^1Elite");
		count++;
	}
	if( strstr( g_admin_admins[ i ]->flags, va( "%s", ADMF_SCRIMMER ) ) && count == 0 )
	{
		strcat(ranks, "^7a ^3Scrimmer");
		count++; 
	}
	...
	if( strstr( g_admin_admins[ i ]->flags, va( "%s", ADMF_EVENTORGANIZER ) ) )
	{
		...
		strcat(ranks, "^7an ^6Event Organizer");
	...
	if( strstr( g_admin_admins[ i ]->flags, va( "%s", ADMF_2NDCOMMAND ) ) )
	{
		...
		strcat(ranks, " ^5Second in Command");
	...
	if( strstr( g_admin_admins[ i ]->flags, va( "%s", ADMF_MENTORCOORDINATOR ) ) )
	{
		...
		strcat(ranks, " ^2Mentor Coordinator");
	...
	if( strstr( g_admin_admins[ i ]->flags, va( "%s", ADMF_RANKINGOFFICER ) ) )
	{
		...
		strcat(ranks, " ^5Ranking Officer");

		count++;
	}
	if( strstr( g_admin_admins[ i ]->flags, va( "%s", ADMF_GENERALOFARMY ) ) && milcount == 0 )
	{
		strcat(military, "a ^1General of the Army ");
		milcount++; 
	}
	if( strstr( g_admin_admins[ i ]->flags, va( "%s", ADMF_LIEUTENANTGENERAL ) ) && milcount == 0 )
	{
		strcat(military, "a ^4Lieutenant General");
		milcount++; 
	}
	if( strstr( g_admin_admins[ i ]->flags, va( "%s", ADMF_COLONEL ) ) && milcount == 0 )
	{
		strcat(military, "a ^5Colonel");
		milcount++; 
	}
	if( strstr( g_admin_admins[ i ]->flags, va( "%s", ADMF_MAJOR ) ) && milcount == 0 )
	{
		strcat(military, "a ^6Major");
		milcount++; 
	}
	if( strstr( g_admin_admins[ i ]->flags, va( "%s", ADMF_CAPTAIN ) ) && milcount == 0 )
	{
		strcat(military, "a ^2Captain");
		milcount++; 
	}
	if( strstr( g_admin_admins[ i ]->flags, va( "%s", ADMF_1STLIEUTENANT ) ) && milcount == 0 )
	{
		strcat(military, "a ^3First Lieutenant");
		milcount++; 
	}
	if( strstr( g_admin_admins[ i ]->flags, va( "%s", ADMF_2NDLIEUTENANT ) ) && milcount == 0 )
	{
		strcat(military, "a ^4Second Lieutenant");
		milcount++; 
	}
	if( strstr( g_admin_admins[ i ]->flags, va( "%s", ADMF_SERGEANT ) ) && milcount == 0 )
	{
		strcat(military, "a ^5Sergeant");
		milcount++; 
	}
	if( strstr( g_admin_admins[ i ]->flags, va( "%s", ADMF_CORPORAL ) ) && milcount == 0 )
	{
		strcat(military, "a ^6Corporal");
		milcount++; 
	}
	if( strstr( g_admin_admins[ i ]->flags, va( "%s", ADMF_PRIVATE ) ) && milcount == 0 )
	{
		strcat(military, "a ^6Private");
		milcount++; 
	}
  }
	...
	if(count == 0 && milcount == 0)
	{
     AP( va( "print \"^3!ranktest: ^7%s^7 is currently a nub^7.\n\"",
		 ent->client->pers.netname ));
	}     
...
if( strstr( g_admin_admins[ i ]->flags, va( "%s", ADMF_EVENTORGANIZER ) ) )
{
   if( G_SayArgc() < 2 + skiparg )
  {
    ADMP( "^3!rw: ^7usage: !rw [map] [1|0]\n" );
	ADMP( "^3!rw: ^7usage: !rw [1|0]\n" );
       return qfalse;
  }
  G_SayArgv( skiparg + 1, map, sizeof( map ) );
  if( map[ 0 ] == '1' )
  {
	AP( "print \"^3RantWrestleMode:""\n");
   trap_Cvar_Set( "g_gravity", "200" );
  ...
  if( map[ 0 ] == '0' )
  {
   ...
   trap_Cvar_Set( "g_humanStage", "0" ); // 1 means stage 2 etc.
   return qtrue;
  } 
  if( !trap_FS_FOpenFile( va( "maps/%s.bsp", map ), NULL, FS_READ ) )
  {
  ...
  {
    ADMP( va( "^3!rw: ^7invalid character\"%c\"\n", number[0] ) );
    return qfalse;
  }
  if (mode == 1)
  {
...
{
char stage_chr[ MAX_STRING_CHARS ];
char team_chr[2];
int minargc;
int stage;
int realStage;
int maxStage = 2;
int minStage = 0;
qboolean tAliens = qtrue;
qboolean tHumans = qtrue;
